...
const postData = async (data) => {
  const query = `
      INSERT INTO foodlist (image, subImage1, subImage2, resName, foodName, tag, price, comment, address, latitude, longitude, rating, category)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
...
const updateRow = async (data) => {
  try {
    const query = `
      UPDATE foodlist 
      SET image = ?, subImage1 = ?, subImage2 = ?, resName = ?, foodName = ?, tag = ?, price = ?, comment = ?, address = ?, latitude = ?, longitude = ?, rating = ?, category = ?
      WHERE id = ?
    `;
    const values = [
      data.image,
